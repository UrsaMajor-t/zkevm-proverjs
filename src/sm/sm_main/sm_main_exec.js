...
module.exports = async function execute(pols, input, rom, config = {}, metadata = {}) {
    ...
    try {
        ...
        const P2_BITS = Math.log2(N);
        console.log("P2_BITS", P2_BITS);
        const JMPN_COND_MASK = (2n ** BigInt(P2_BITS)) - 1n;
        const JMPN_COND_VALUE_BITS = BigInt(P2_BITS);

        const H_JMPN_COND_VALUE_BITS_LENGTH = pols.hJmpnCondValueBit.length;
        console.log("H_JMPN_COND_VALUE_BITS_LENGTH", H_JMPN_COND_VALUE_BITS_LENGTH);
        for (let step = 0; step < stepsN; step++) {
            ...
            if (false) {
                ...
                console.log("ctx.Step ", ctx.step);
                console.log("ctx.cntStep ", ctx.cntStep);
            }
    ...
    if (!(fastDebugExit || typeof config.stepsN === 'undefined')) {
        console.log("checkFinalState", checkFinalState);
